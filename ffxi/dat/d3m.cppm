...
D3A::D3A(char* _name, uint8_t* _buffer, size_t _len) : DatChunk(_name, _buffer, _len)
{
    ...
    // not sure what the other values are? num_quads seems to be repeated at 0x05 and 0x06 (but doesn't do anything),
    // and 0x07 is 1?
...
lotus::Task<> D3MLoader::InitPipeline(lotus::Engine* engine)
{
    ...
    std::vector<vk::PipelineColorBlendAttachmentState> color_blend_attachment_states_subpass1{
        color_blend_attachment_accumulation, color_blend_attachment_revealage, color_blend_attachment_particle};
