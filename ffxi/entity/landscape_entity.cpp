...
lotus::WorkerTask<> FFXILandscapeEntity::Load(std::shared_ptr<lotus::Entity> entity, lotus::Engine* engine, size_t zoneid, lotus::Scene* scene)
{
    ...
    if (mzb)
    {
        ...
        if (collision_model_task)
            co_await *collision_model_task;
    }
    co_return;
}
...
{
    current_time = std::chrono::duration_cast<FFXITime::milliseconds>((FFXITime::vana_time() %
FFXITime::days(1))).count() / 60000.f; current_time = 860;

    auto elapsed = time - create_time;
    auto frame = (std::chrono::duration_cast<std::chrono::milliseconds>(elapsed) * 7.5f) /
