...
lotus::Task<> SchedulerComponent::tick(lotus::time_point time, lotus::duration delta)
{
    ...
    while (frame_number > next_frame && !finished)
    {
        ...
        case 0x2d:
        {
            ...
            {
                auto s = entity->getComponent<SchedulerComponent>([current_scheduler](SchedulerComponent* c) { return
            c->getName() == current_scheduler; }); if (s)
                {
        ...
        case 0x5f:
        {
            ...
            auto s = entity->getComponent<SchedulerComponent>([current_scheduler](SchedulerComponent* c) { return
            c->getName() == current_scheduler; }); s->cancel();
            */
        }
        break;
        }
...
void SchedulerComponent::cancel() { remove(); }
} // namespace FFXI
