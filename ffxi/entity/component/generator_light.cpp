...
import :entity.component.generator_light;
...
lotus::Task<> GeneratorLightComponent::init()
{
    ...
    if (glm::length(pos - glm::vec3(419, -53.f, -103.f)) < 10.f)
    {
        ...
        light = engine->lights->AddLight({.pos = pos, .intensity = 100.f, .colour = colour, .radius = 0.5f});
    }
...
lotus::Task<> GeneratorLightComponent::tick(lotus::time_point time, lotus::duration delta)
{
    ...
    if (light)
    {
        engine->lights->UpdateLight(light, {.pos = pos, .intensity = 300.f, .colour = colour, .radius = 0.5f});
    }
    co_return;
}
} // namespace FFXI
