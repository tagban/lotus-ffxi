...
lotus::Task<std::pair<std::shared_ptr<lotus::Entity>, Actor::InitComponents>> Actor::Init(lotus::Engine* engine, lotus::Scene* scene, uint16_t modelid)
{
    ...
    std::vector<std::reference_wrapper<const FFXI::Dat>> dats = {static_cast<FFXIGame*>(engine->game)->dat_loader->GetDat(dat_index)};
...
lotus::WorkerTask<Actor::InitComponents> Actor::Load(std::shared_ptr<lotus::Entity> actor, lotus::Engine* engine, lotus::Scene* scene,
                                                     std::shared_ptr<const FFXI::ActorSkeletonStatic> static_skeleton,
                                                     std::variant<FFXI::ActorSkeletonComponent::LookData, uint16_t> look,
                                                     std::vector<std::reference_wrapper<const FFXI::Dat>> dats)
{
...
/*
lotus::WorkerTask<std::tuple<FFXI::ActorPCModelsComponent*>> Actor::LoadPC(std::shared_ptr<lotus::Entity> entity,
lotus::Engine* engine, lotus::Scene* scene, InitComponents components, FFXI::ActorPCModelsComponent::LookData look,
std::initializer_list<std::reference_wrapper<const FFXI::Dat>> dats)
{
    auto pc_c = co_await FFXI::ActorPCModelsComponent::make_component(entity.get(), engine,
*std::get<lotus::Component::DeformedMeshComponent*>(components),
        *std::get<lotus::Component::AnimationComponent*>(components),
std::get<lotus::Component::DeformableRaytraceComponent*>(components), look); co_return
scene->AddComponents(std::move(pc_c));
}
*/
